---
title: "Maestría en Ciencias de los Alimentos y Salud Humana"
similar:
  - ciencias-odontologicas-opcion-salud-colectiva-y-epidemiologia-bucal-maestria-udelar
  - administracion-de-servicios-de-salud-especializacion-udelar
  - investigacion-de-la-salud-con-enfasis-en-perinatologia-especializacion-udelar
  - psicoterapia-en-los-servicios-de-salud-especializacion-udelar
  - salud-ocupacional-especializacion-udelar
  - salud-publica-especializacion-udelar
institutionName: "Universidad de la República (UDELAR)"
institution: "udelar"
degreeType: "maestria"
area: "Ciencias de la Salud"
modality: "presencial"
shift: "day"
weeklyHours: "No especificado"
cost: "Arancelada"
language: "Español"
website: "https://cap.posgrados.udelar.edu.uy/detallesPosgrado.php?idServicio=9&idPosgrado=70"
location: "Facultad de Medicina"
description: "Maestría en Ciencias de los Alimentos y Salud Humana, dictada por la Facultad de Medicina."
startDate: ""
applicationDeadline: ""
tags:
  - alimentos
  - salud publica
draft: true
createdAt: "2026-07-31 16:58:06"
updatedAt: "2026-08-04 19:47:33"
sources:
  - label: 'Catálogo de Posgrados Udelar (CAP)'
    url: 'https://cap.posgrados.udelar.edu.uy/detallesPosgrado.php?idServicio=9&idPosgrado=70'
  - label: 'Ficha PDF'
    url: 'https://cap.posgrados.udelar.edu.uy/fpdf/posgradoPDF.php?idServicio=9&idPosgrado=70'
---




## Resumen

Información no disponible en el catálogo de posgrados de Udelar.
